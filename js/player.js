...
function initializePageContent() {
    ...
    // 设置页面标题
    document.title = currentVideoTitle + ' - 6789TV播放器';
...
async function showSwitchResourceModal() {
    ...
    for (const [sourceKey, result] of sortedResults) {
        ...
        const coverBaseUrl = result.source_api_url || result.api_url || '';
        const coverImage = getPosterImageSources(result.vod_pic, sourceKey, coverBaseUrl);
        const safeResultName = escapeAttr(result.vod_name || '');
        ...
        html += `
            <div class="relative group ${isCurrentSource ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:scale-105 transition-transform'}" 
                ...
                <div class="aspect-[2/3] rounded-lg overflow-hidden bg-gray-800">
                    <img src="${escapeAttr(coverImage.src)}" 
                         alt="${safeResultName}"
